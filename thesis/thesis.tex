...
% Switch to arabic numbering and start the enumeration of chapters in the table of content.
    ...
    Given the input values $\bar{\mathbf{X}}$ and the weights $\bar{\mathbf{W}}=[w_0,w_1,\ldots,w_d]$, a prediction $\hat{y}$ for $y$ is performed by the following equation.

    \begin{equation}
        \hat{y}= \bar{\mathbf{X}}\cdot\bar{\mathbf{W}}=\sum_{j=0}^{d}x_j*w_j
    \end{equation}

    Lastly, the result of the equation is being activated using an \textit{activation function}.
    For instance, in classification the sign function is used to map the prediction $\hat{y}$ to the previously specified range of $[-1,1]$.
    After the output value has been activated, an error between the prediction and the actual outcome can be calculated, $E(\bar{\mathbf{X}}) = y - \hat{y}.
    If the case that the error is nonzero, the weights must be adapted in order to perform a closer prediction next time.




    \subsubsection{Activation Functions}
    Concerning the sign functionl
