...
% Switch to arabic numbering and start the enumeration of chapters in the table of content.
    ...
    \gls{rl} problems are tackled by a learner or rather an \textit{agent} which performs actions in an \textit{environment} in a trial and error paradigm.
    ...
    This creates the need for a proper weighting between exploration and exploitation as the agent must exploit actions with known consequences to perform well but also explore new ones in order to improve.

    In addition to the aforementioned environment, agent, and reward signal, \gls{rl} consists of further subelements since e.g. it would be desirable to not only know the immediate reward but to also have a measurement for all possible future rewards from a given state onwards.
    For this, a \textit{value function} is used which acts as a long-term indicator that describes the attractiveness of a state an agent might result in after it interacted with its environment.
    ...
    Lastly, the agent might want to learn a \textit{model} of its environment which allows it to plan its action ahead since each interaction can result in a different state.
    ...
    This spares the necessity of creating a trainings dataset with corresponding state-action tuples beforehand and thus \gls{rl} is the preferred choice in cases where an agent interacts with and influences its environment.
    Furthermore, teaching based on supervision would mean that an agent would be limited to the labelling ability of the supervisor which is another reason why it must be able to learn from its own experiences~\citep{silver_rl_nodate}.
    ...
    After the agent selected an action, it obtains a scalar reward, $R_{t+1} \in \mathit{R} \subset \mathbb{R}$ and depending on the action the environment transitions into a subsequent state, $S_{t+1}$.
    Through continuous interactions between the agent and the environment a trajectory of the form $S_0,A_0,R_1,S_1,A_1,R_2,S_2,A_2,R_3,\ldots$ is created.
    ...
    In a finite-horizon, the agent has to plan only a fix number of time-steps ahead and the sets $\mathit{S},\mathit{A}, and \mathit{R}$ are limited~\citep{kaelbling_reinforcement_1996,sutton_reinforcement_2018}.
    Furthermore, the possibility of occurrence a time step $t+1$ of the values $s'\in \mathit{S}$ and $r \in \mathit{R}$ can be described with a probability~\eqref{eq:distribution} only depending on the action $a \in \mathit{A}(s)$ taken in a preceding state $s \in \mathit{S}$.

    \begin{equation}
        p(s',r|s,a) = P\{S_{t+1}=s', R_{t+1}=r | S_t=s, A_t=a\}\label{eq:distribution}
    \end{equation}

    Since $p$ describes a probability the sum over all possible combination of $s$ and $a$ must be equal to 1.
    Moreover, the individual probabilities of $p$ describes the dynamics of an \gls{mdp}.
    Observing this notation shows that the preceding state $s$ must contain all the necessary information and must not depend on states preceding $s$.
    If this is the case, the a state is called to have the Markov property~\eqref{eq:markov_property}~\citep{silver_rl_nodate}.
    ...
    As mentioned in the \refname{sec:reinforcement-learning-problem}, rewards at each time-step, $R_t \in \mathbb{R}$, are used to formalize a goal which the agent should pursue.
    This is possible due to the rewards hypothesis which states that every goal one can think of, can be formalized through a numeric reward signal and achieved by maximizing the cumulative long-term rewards~\citep{sutton_reinforcement_2018}.
    ...
    However, this notation is only applicable if an environment has a identifiable terminal state.
    When this state is reached, it breaks a e.g.\ game sequence into multiple ones, where each one is called an episode.
    Independent of how the agent reached the final state, the environment will reset and start over with an default initial state.

    Sometimes this separation of episodes might not be as clear and the sequence's limit is infinite.
    In opposite to episodic task where $T$ is finite, this is called an continuing task.
    ...
    This issue is solved, by introducing a discounting rate $\gamma$, $0 \leq \gamma \leq 1$~\citep{sutton_reinforcement_2018} which also avoids infinite rewards caused by cycle in an \gls{mdp}.
    ...
    With $\gamma$ as newly introduced hyperparamter, the weighting of the rewards can be adjusted according to the given task.
    A low $\gamma$ means that the agent values the immediate reward and rewards in the near future more.
    On the other hand, a high $\gamma$ forces the agent to also consider long-term future rewards and thus be more far-sighted.


    \section{Functions to Improve the Policy}
    As mentioned in \refname{sec:reinforcement-learning-problem}, the policy is an important component of the \gls{rl} framework and is usually denoted as $\pi$.
    By following this policy, the agent's probability to select a specific action at time $t$ is $\pi(a|s) = P\{A_t=a|St=s\}$.
    ...
    Formally, the value or state-value function for an \gls{mdp} is denoted by $v_\pi(s)$ which stands for the expected future rewards when following the policy, starting in state $s$~\cite{sutton_reinforcement_2018}.

    \begin{equation}
        v_\pi(s) = \mathbb{E}_\pi[G_t|S_t = s] = \mathbb{E}_\pi \Bigg[\sum_{k=1}^{\infty} \gamma^k R_{t+k} \bigg| S_t = s \Bigg]
    \end{equation}
    ...
    \begin{equation}
        q_\pi(s,a) = \mathbb{E}_\pi[G_t|S_t = s, A_t = a] = \mathbb{E}_\pi \Bigg[\sum_{k=1}^{\infty} \gamma^k R_{t+k} \bigg| S_t = s | A_t = a \Bigg]
    \end{equation}
