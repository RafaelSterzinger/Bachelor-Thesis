...
% Switch to arabic numbering and start the enumeration of chapters in the table of content.
    ...
    If this is the case, the a state is called to have the Markov property~\eqref{eq:markov_property}~\citep{silver_lecture_2015}.
    ...
    By following a policy, the agent's probability to select a specific action at time $t$ is $\pi(a|s) = P\{A_t=a|St=s\}$ and therefore $\pi$ describes a probability distribution over actions given states~\citep{silver_lecture_2015-1}.
    ...
    If a policy $\pi$ achieves a higher value than another policy $\pi'$, $v_\pi(s) \geq v_{\pi'}(s), \forall s \in \mathit{S}$, then it can be ordered partially, $\pi \geq \pi'$.
    ...
    An very simple optimal policy can be derived through the action-value function $q_*(s,a)$ by simply selecting the action with the maximum value.
